...
class NetworkManager {
    ...
    private struct Components {
        ...
        static let placeNamePath = "/data/2.5/weather"
        ...
        static let languageItem = URLQueryItem(name: "lang", value: NSLocalizedString("language_request",
                                                                                      comment: ""))
    ...
    static func getPlaceName(for coordinates: Coordinates, completion: @escaping NetworkCompletionHandler) {
        guard let url = getURL(forPlaceNameWith: coordinates) else {
            print("ERROR")
            completion(nil, .wrongURL)
            return
        }
    ...
    static func getURL(forPlaceNameWith coordinates: Coordinates) -> URL? {
        var components = URLComponents()
        components.scheme = Components.scheme
        components.host = Components.host
        components.path = Components.placeNamePath
